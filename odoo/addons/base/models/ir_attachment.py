...
class IrAttachment(models.Model):
    ...
    def _full_path(self, path):
        ...
        path = re.sub('[.:]', '', path)
        path = path.strip('/\\')
        full_path = os.path.join(self._filestore(), path)
        # 在Windows上，确保路径分隔符正确
        if os.name == 'nt' and '/' in full_path:
    ...
    def _to_http_stream(self):
        ...
        if self.store_fname:
            ...
            if os.name == 'nt':  # Windows
                # 先替换为正斜杠，因为safe_join使用posixpath
            ...
            if stream.path and os.name == 'nt':
                # 在Windows上将正斜杠转换回反斜杠
                stream.path = stream.path.replace('/', '\\')
