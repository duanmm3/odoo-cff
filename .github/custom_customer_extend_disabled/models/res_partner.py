...
class ResPartner(models.Model):
    ...
    # 客户等级
    customer_grade = fields.Char(string='客户等级', help='例如: 0,5a,5b,5c,10a,10b,10c')
    ...
    def _generate_partner_code(self, vals):
        ...
        # 当partner_type为'both'时，优先使用客户类型
        if partner_type == 'both':
            # 优先使用客户类型
            if vals.get('customer_type'):
                prefix = vals.get('customer_type', 'CF')
                field_to_check = 'customer_type'
            else:
                prefix = vals.get('supplier_type', 'ST')
                field_to_check = 'supplier_type'
        elif partner_type in ['customer', 'both']:
            prefix = vals.get('customer_type', 'CF')
            field_to_check = 'customer_type'
        elif partner_type in ['supplier', 'both']:
            prefix = vals.get('supplier_type', 'ST')
            field_to_check = 'supplier_type'
        else:
            prefix = 'OTH'
            field_to_check = 'partner_type'
        
        # 使用self.env来搜索，而不是self.search()
        domain = []
        if field_to_check == 'customer_type':
            domain = [('customer_type', '=', prefix), ('partner_code', 'like', f'{prefix}%')]
        elif field_to_check == 'supplier_type':
            domain = [('supplier_type', '=', prefix), ('partner_code', 'like', f'{prefix}%')]
        elif field_to_check == 'partner_type':
            domain = [('partner_type', '=', 'other'), ('partner_code', 'like', f'{prefix}%')]
        
        existing_codes = self.env['res.partner'].search(domain).mapped('partner_code')
    ...
    def default_get(self, fields):
        ...
        res['shared_users'] = [(6, 0, [self.env.user.id])]
        return res
    ...
    def search(self, domain, offset=0, limit=None, order=None, count=False):
        ...
        if self.env.uid != 1:
            ...
            domain = domain + [('create_uid', '=', user_id)]
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        if self.env.uid != 1:
            if domain is None:
                domain = []
            domain = domain + [('create_uid', '=', self.env.user.id)]
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        """
        import sys
        print(f'====== search_read: uid={self.env.uid}, user={self.env.user.name}', file=sys.stderr)
        ...
        if self.env.uid != 1:
            ...
            if domain is None:
                domain = []
            domain = domain + [('create_uid', '=', user_id)]
            print(f'====== search_read: 添加过滤 user_id={user_id}', file=sys.stderr)
